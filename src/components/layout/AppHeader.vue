...
<template>
  <header class="flex items-center justify-between h-12 px-4 select-none shrink-0 bg-surface border-b border-border"
    @mousedown="handleHeaderMouseDown">
    <!-- 左侧: Logo + 名称 -->
    <div class="flex items-center gap-3 shrink-0">
      <div class="w-7 h-7 rounded-md flex items-center justify-center bg-primary">
    ...
    <div class="flex items-center gap-1 shrink-0">
      <div class="flex items-center ml-1" @mousedown.stop>
        ...
        <Button variant="ghost" size="icon" class="text-text hover:bg-hover" @click="appWindow.minimize()" title="最小化">
          <Minus class="w-4 h-4" />
        </Button>
        <Button variant="ghost" size="icon" class="text-text hover:bg-hover" @click="appWindow.toggleMaximize()"
          title="最大化">
          <Square class="w-3.5 h-3.5" />
        </Button>
        <Button variant="ghost" size="icon" class="text-text hover:bg-red-500/80" @click="appWindow.close()" title="关闭">
          <X class="w-4 h-4" />
        </Button>
      </div>
    </div>
